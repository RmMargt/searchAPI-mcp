...
SEARCHAPI_API_KEY = os.environ.get(
    "SEARCHAPI_API_KEY"
)  # 重要：运行前请设置环境变量 SEARCHAPI_API_KEY
...
async def search_google_flights(
    ...
    """搜索Google航班信息"""
    params = {"engine": "google_flights", "flight_type": flight_type}
...
@mcp.tool()
async def search_google_hotels_autocomplete(q: str, hl: str = None) -> Dict[str, Any]:
    """Google Hotels 自动补全"""
    params = {"engine": "google_hotels_autocomplete", "q": q}
...
async def search_google_flights_location_search(
    q: str, search_type: str = None, hl: str = None
) -> Dict[str, Any]:
    """Google Flights 位置搜索"""
    params = {"engine": "google_flights_location_search", "q": q}

    optional_params = {"search_type": search_type, "hl": hl}
...
@mcp.tool()
async def search_google_maps_photos(
    place_id: str = None,
    data_id: str = None,
    hl: str = None,
    ...
    """获取 Google Maps 地点照片"""
    params = {"engine": "google_maps_photos"}
...
async def search_google_maps_place(
    place_id: str = None, data_id: str = None, google_domain: str = None, hl: str = None
) -> Dict[str, Any]:
    """获取 Google Maps 地点详情"""
    params = {"engine": "google_maps_place"}
    ...
    optional_params = {"google_domain": google_domain, "hl": hl}
...
async def get_current_time(
    ...
    return_future_dates_bool = (
        return_future_dates.lower() == "true" if return_future_dates is not None else False
    )
    ...
    result["travel_dates"] = {
        ...
        "weekend": (now + timedelta((5 - now.weekday()) % 7)).strftime(
            "%Y-%m-%d"
        ),  # 下一个周五
        "weekend_end": (now + timedelta((6 - now.weekday()) % 7 + 1)).strftime(
            "%Y-%m-%d"
        ),  # 下一个周日
    }
...
async def search_google_videos(
    ...
    params = {"engine": "google_videos", "q": q}
